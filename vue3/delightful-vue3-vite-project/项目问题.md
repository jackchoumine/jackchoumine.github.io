...
[浅谈使用卫语句重构方法](https://codeantenna.com/a/ZBV9HWDiBj)
[以卫语句取代嵌套表达式](http://121.36.11.93/best_code/refactoring/simplify_logic/replace_nested/)
[Java规约-卫语句(guard clauses)](http://www.houxiurong.com/Java/46.html)

```js
function toImgName({
  count
}) {
  // 不提前返回  if-else if 语句有优先级
  let imgName = 3
  if (count >= 30) {
    imgName = 1
  } else if (count >= 11) {
...
function toImgNameV1({
  count
}) {
  // 提前返回  if-else if 语句有优先级
  if (count >= 30) {
    return 1
  } else if (count >= 11) {
    return 2
...
function toImgNameV3({
  count
}) {
  // 提前返回  if-else 语句有优先级，判断条件复杂
  if (count === 0) {
    return 3
...
function toImgNameV4({
  count
}) {
  // 提前返回  if 语句无优先级，判断条件复杂
  if (count === 0) {
    return 3
  }
  if (count >= 1 && count < 10) {
...
> 提前返回，函数执行路径更加短，提前返回的更好。 `if` 语句有优先级，意味着需要添加一个条件时，容易引发 bug，避免它代价是判断条件变得复杂，同时条件也更加明确，所以 `if` 语句无优先级的更加好理解。综上 `toImgNameV4` 更加可读。
