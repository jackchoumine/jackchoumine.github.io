...
如何测试组件的 props , 是一个很重要的问题。好的测试方式可以让代码更加容易维护，验证逻辑和UI逻辑分离。
...
造成这个问题的根源是我们的组件没有分离关注点 -- 把 props 的验证逻辑和 UI 逻辑耦合在一起。
...
3. 测试组件的行为：根据组件输入，测试输出，而不是测试组件的实现细节，组件重构时，不会让已有的测试失效；
4. 测试一个函数是否抛错：`箭头函数` + `toThrow`；
5. 理解和找到关注点的分界线，并合理组织不同关注点的逻辑，对写好易维护的代码至关重要；
6. 通过代码是否容易测试，可反推出代码设计得是否合理，关注点是否耦合或者分离了。
