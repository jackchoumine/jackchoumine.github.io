...
最近阅读了《编写可读代码的艺术》一书，感觉很有收获，现在结合自己的理解再来总结编写可读性代码的技巧，会用 js 举例，并且针对日常开发中常见的**代码异味**给出我的改进建议。

学会该书的大部分技巧并付诸实践，无法保证你立马写出完美的代码，但能保证你写出可读性不是非常糟糕的代码，保证你的**码德**下限。
...
> 代码可读性和代码被他人理解的时间成反比，即他人**理解**代码的时间越少，可读性越高。
...
> 通过使用 `is` 、 `can` 、 `should` 、 `enable` 和 `has` 前缀或者 `able` 后缀表示布尔变量。
...
| 后缀 | 常见的搭配      | 描述                           | 例子                                                              | 补充           |
| ---- | --------------- | ------------------------------ | ----------------------------------------------------------------- | -------------- |
| able | 动词原型 + able | 是否具备某种能力，是否支持 xxx | sortable(是否可排序)、selectable(是否可选中)、writeable(是否可写) | 常用于变量命名 |
...
数学上表示所有的的量词叫全称量词，表示存在的量词叫存在量词。如何命名呢？
...
1.  对象中的 key 的顺序和 html 中表单的顺序一致；<br/>
2.  使用者最关心的在前，不太关心的在后，比如 css 属性；<br/>
3.  必需的在前，可选的在后，比如后端接口验证前端提交的数据；<br/>
4.  和代码的生命周期一致，有些代码有生命周期，比如 vue 组件，调用这些声明周期钩子函数时，最好按照生命周期的顺序调用；<br/>
5.  按照字母排序。
    <br/>
    <br/>
...
阅读一个函数时，更加关注函数的**名字**、**参数**和**返回值**，因为这三要素决定了函数的用法。useHover 函数没有把返回值放在前面，需要滚动到后面才知道返回值。把返回值提前，让阅读者快速知道它的存在。
...
这个例子中，return 语句之前的代码只有一个函数，代码行数不是很多，把 return 语句放在最后，对可读性影响不大，但是当 return 语句之前有很多代码时，把返回值提前，可让读者快速看到返回值，而不用滚动到函数末尾。

> js 使用 `function` 定义函数，允许调用在前，定义在后，使用 `const let` 和函数声明定义函数，不支持调用在前，声明在后。

> 经验法则：使用 function 定义函数，因为它允许先调用后定义，function 也快速让读者知道它是一个函数，而不是其他类型的变量。

> 经验法则：一个 js 文件有多于**3 个以上**的导出，统一放在**文件底部**导出。

> 经验法则：相似的代码，选择一个有意义的顺序，并保持一致。一段代码中 `A` 、 `B` 和 `C` ，在另一段代码中 `C` 、 `B` 和 `A` 会降低可读性。保持顺序一致，也符合让功能相似的代码看起来也相似的原则。
...
> 这个注释告诉了读者，为何使用使用二叉树，防止他无谓的优化而浪费时间。
...
第一个更加易读，因为它和英文用法一样，"长度大于 10"，中文语义也是一样的。

通用的指导原则：

| 比较的左侧           | 比较的右侧                                   |
| -------------------- | -------------------------------------------- |
| **变量**，不断变化的 | 用来作比较的表达式，它的值更加倾向于**常量** |

### 尤达条件 vs 变量在前的条件

上述条件中，变量在条件表示前面，优点是符合人类语言习惯，可读性高，缺点是在相等比较中容易出错，因为 `=` 和 `==` 很容易混淆，此时就会意外改写变量的值，造成 bug。

```js
let a = 1
if(a=1){ // ❌ 当 不小心把 == 写成 =，程序不报错
  // do something
}
```
...
```js
let b = '123'
if('12' == b){ // 当 不小心把 == 写成 =， 程序报错
  // do something
}
```

> 实践建议：
> 不在条件语句中进行赋值： ESLint 语法检查器，可以警告条件式中出现赋值操作。
> 使用变量在右边的条件 + const + ESLint 能最大程度避免其缺点。
...
1.  让函数变得不纯；
2.  中断程序控制流，从而会中断理解思路，可读性不高，难以推理程序执行流程；
3.  语法冗余，模板代码多；
4.  程序中断后无法恢复。
...
1.  避免输入错误。第一个版本有一个单词写错了(highighted 少了一个字母 l)
2.  当名字需要修改，只改一处。
3.  降低了行宽。
...
缩小变量作用域是提高可读性的重要手段，常说的**减少全局变量的使用**，就是一种缩小变量作用域的手段。
...
1.  使用`immutable.js`等不可变的数据结构；
2.  使用 js 库(比如 lodash)来执行不可变的操作；
3.  使用 es6 中执行不可变操作；
4.  赋值一次，不再赋值的变量，使用`const`声明；
5.  编写无副作用的函数：① 函数不改变参数；② 不抛错；③ 必需有返回值；
6.  涉及到传递对象和数组时，传入深度复制后的数据(严格来说，不是不可变)。
...
> 函数参数不应该超过 4 个，超过 4 个，就难以理解和使用。超过 4 个，使用对象代替。
...
柯里化的基本形式：函数 A 返回另函数 B，B 使用 A 的参数参与计算。
...
了解常用的周边库，掌握使用技巧，使很有必要的。使用广泛的库，经过社区千锤百炼，不太可能出现大问题，能帮你又快又好的解决问题。

如何选择你使用的库？或者如何做技术选型？这个话题可单独写一篇文章了。
...
2.  为特殊的输入，常见的 bug，编写用例，让使用者了解意外情况。
...
1.  提取可复用的操作
2.  保持代码块（函数、类、代码段落）的职责单一
3.  善用周边库，少造轮子，保持代码库精简
