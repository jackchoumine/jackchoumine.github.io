...
| 后缀   | 常见的搭配 | 描述 | 例子 | 补充 |
| ------ | ------------- | ---------- | -------------- | ------------------------- |
| able | 动词原型 + able | 是否具备某种能力，是否支持xxx | sortable(是否可排序)、selectable(是否可选中)、writeable(是否可写)| 常用于变量命名|
...
05.  编写无副作用的函数：① 函数不改变参数；② 不抛错；③ 必需有返回值；
06.  涉及到传递对象和数组时，传入深度复制后的数据(严格来说，不是不可变)。
