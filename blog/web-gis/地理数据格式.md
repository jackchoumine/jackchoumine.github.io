...
带洞的面, Coordinates 中第一个点串行表示**外环**，第二个及其他表示内环。 带洞的多边形使用场景很多，比如水系相关表达河流三角洲等等。
...
地图上的形状也应该有一些现实世界的意义。此含义由该形状的属性定义。
...
FeatureCollection 对象是 Feature 对象的组合。它是你将在 GeoJSON 文档中观察到的最常见的顶级结构。它有一个成员“type”，其值为“FeatureCollection”，另一个成员名为“features”，其值为 Feature 对象数组，可为空。一般不会混合定义，分开定义方便操作。
