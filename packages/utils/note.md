...
> umd 模块，是一种兼容 amd 、 commonjs 和 iife 模块的规范，这种库可在浏览器和 node 环境中使用。

2. npm 安装 -- cjs 模块

开发者使用 `npm` 等依赖管理工具安装库， 通过 `package.json` 对库的依赖和版本进行说明，然后使用 `require` 引入，这种方式不需要手动管理依赖关系，但要求库遵循 `commonjs` 规范编写，这种库不能直接在浏览器中使用，需要使用 `rollup` 、 `webpack` 等工具转换成浏览器支持的代码。
...
加载一个库时，这些工具会自动解析 `package.json` 中的 `type` 和 `module` 字段，type 字段为 `module` ， 表明是 `esm` 规范的库， `module` 字段指定 esm 的入口。type 为 `commonjs` ，表示是 cjs 模块， main 字段指定了库的入口文件。打包工具会根据这些信息，加载相关的文件。
...
为了满足不同的使用方式，需要借助一些工具，，比如 `rollup` 、 `webpack` 等，把库打包成不同的模块化规范。
...
> 既然 cjs 和 umd 兼容，为何需要两者，因为 cjs 对**条件导入**支持更好。
...
都不是，放在 `peerDependencies` ，默认使用者的环境中已经有这些依赖，不需要重复安装。

比如，开发一个 react UI 库，就可以把 react 和 react-dom 放在 peerDependencies 中。
...
> peerDependencies 的版本最好指定一个较为宽泛的范围，以覆盖更多的版本，降低潜在的版本冲突。
...
[es module 与 commonjs 的异同点是什么？](https://mp.weixin.qq.com/s/MqoIa80IpvRDfOA2EahBdQ)

[制作一个同时支持 esm, cjs, Browser 的 Package](https://juejin.cn/post/7129335434588454919)

[NPM Package: How to Conditionally Include Developer Tools?](https://tolgee.io/blog/conditional-export)
